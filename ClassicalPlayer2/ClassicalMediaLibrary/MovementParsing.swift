...
fileprivate func apply(expression: NSRegularExpression, to raw: String) -> (firstMatch: String, secondMatch: String)? {
    ...
    guard let firstResult = checkingResult.first else {
        return nil
    }
    guard firstResult.numberOfRanges > 1 else {
    ...
    let firstComponent = extract(from: raw, range: firstResult.range(at: 1))
    let secondComponent: String
    if firstResult.numberOfRanges > 2 {
        secondComponent = extract(from: raw, range: firstResult.range(at: 2))
    } else {
        secondComponent = ""
    }
...
fileprivate func extract(from: String, range: NSRange) -> String {
    ...
    guard let startIndex = from.index(from.startIndex,
                                      offsetBy: range.location,
                                      limitedBy: from.endIndex) else { return "" }
    guard let endIndex = from.index(startIndex,
